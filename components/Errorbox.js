...
export default function Errorbox({title, message, code, options, action}) {
    return(
        <Popup open={true}>
                ...
                {code == 403 && <Link><a href="/" style={{textDecoration: "underline", color: "var(--link)"}}>Go back home!</a></Link>}
                {options?.includes("reload") && <Link><a href="/" className={styles.reload}><i className="fas fa-redo"/>&nbsp; Try again </a></Link>}
